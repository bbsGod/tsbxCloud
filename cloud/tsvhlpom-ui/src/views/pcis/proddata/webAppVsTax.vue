<template>
  <div class="app-container">
    <span>提示：修改webAppVsTax的c_tax_location_code（纳税地区代码）字段</span>
    <el-row> </el-row>
    ...
    <el-row :gutter="10" class="mb8">
      <!--<el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['proddata:webAppVsTax:save']"
        >新增</el-button>
      </el-col>-->
      <!--<el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['proddata:webAppVsTax:update']"
        >修改</el-button>
      </el-col>-->
      <!--<el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['proddata:webAppVsTax:delete']"
        >删除</el-button>
      </el-col>-->
    </el-row>

    <el-table v-loading="loading" :data="webAppVsTaxList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="申请单号" align="center" prop="c_app_no" />
      <el-table-column label="纳税地区代码" align="center" prop="c_tax_location_code" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="800px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="150px">
        <el-form-item label="申请单号" prop="c_app_no">
          <el-input disabled v-model="form.c_app_no" placeholder="请输入申请单号" />
        </el-form-item>
        <el-form-item label="纳税地区代码" prop="c_tax_location_code">
          <el-input v-model="form.c_tax_location_code" placeholder="请输入纳税地区代码" />
        </el-form-item>
      </el-form>
...
import {getWebAppVsTaxList, update} from '@/api/proddata/webAppVsTax'
...
export default {
  ...
  methods: {
    getList() {
      this.loading = true;
      getWebAppVsTaxList(this.queryParams).then(response => {
        this.webAppVsTaxList = response.rows;
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "webAppVsTax新增";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.form.c_app_no = row.c_app_no;
      this.open = true;
      this.title = "webAppVsTax修改";
      //const c_ply_no = row.c_ply_no || this.ids
      /*getwebAppVsTax(c_ply_no).then(response =>{
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          /*if (this.form.c_pk_id != null) {
            updatewebAppVsTax(this.form).then(response =>{
              this.$modal.msgSuccess("更新成功");
              this.open = false;
              this.getList();
            })
          } else {
            savewebAppVsTax(this.form).then(response => {
    ...
    /*handleDelete(row) {
      ...
      this.$confirm('是否确认删除选中的数据?', "警告", {
        ...
        return deletewebAppVsTax(c_pk_ids);
      }).then(() => {
        this.getList();
